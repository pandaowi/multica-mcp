...
export class MulticaClient {
  ...
  async execute(command: { method: string; path: string }, args: Record<string, unknown>, context: ExecutionContext): Promise<ResultEnvelope> {
    ...
    if (!response.ok) { const error = payload && typeof payload === "object" ? payload as Record<string, unknown> : {}; throw new Error(JSON.stringify({ code: error.code ?? (response.status === 401 ? "AUTH_REQUIRED" : response.status === 403 ? "FORBIDDEN" : "UPSTREAM_ERROR"), retryable: response.status >= 500 || response.status === 429, safe_message: error.safe_message ?? "Multica request failed", request_id: requestId })); }
