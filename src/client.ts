...
export interface MulticaClientOptions { baseUrl: string; token?: string; fetchImpl?: typeof fetch; }
export class MulticaClient {
  ...
  async execute(command: { method: string; path: string }, args: Record<string, unknown>, context: ExecutionContext): Promise<ResultEnvelope> {
    ...
    const headers: Record<string, string> = { accept: "application/json", "x-request-id": randomUUID(), "x-connection-id": context.connectionId, "x-principal-id": context.principalId };
    if (body) headers["content-type"] = "application/json";
    if (this.options.token) headers.authorization = `Bearer ${this.options.token}`;
    const response = await this.fetchImpl(url, { method: command.method, headers, body });
    ...
    if (!response.ok) {
      ...
      const safeMessage = typeof error.safe_message === "string" ? redactText(error.safe_message) : "Multica request failed";
      throw new Error(JSON.stringify({ code: typeof error.code === "string" ? error.code : (response.status === 401 ? "AUTH_REQUIRED" : response.status === 403 ? "FORBIDDEN" : "UPSTREAM_ERROR"), retryable: response.status >= 500 || response.status === 429, safe_message: safeMessage, request_id: requestId }));
    }
    return { ...(payload && typeof payload === "object" ? payload as ResultEnvelope : { data: payload }), request_id: requestId, state: "succeeded", warnings: [] };
  }
}
